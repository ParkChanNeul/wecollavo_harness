...
- 브랜드 실행 스프린트 판단 재료: `docs/wecollavo-brand-execution-sprint.md`
...
전략 Skill은 `docs/wecollavo-brand-execution-sprint.md`를 현재 하네스용 판단
재료로 참고한다. 이 문서는 archive PRD에서 추출한 운영 관점이며, 과거 GT 전용
가격, 범위, 사실관계를 일반 고객 기본값으로 일반화하지 않는다.
...
- 고객 요청과 실제 병목은 분리한다.
- WeCollavo는 제작 외주가 아니라 협업형 브랜드 실행 파트너다.
- 고객 요청을 그대로 팔지 않고, 실제로 사려는 변화와 지금 필요한 핵심 브랜드
  자산 1개를 해석한다.
- archive PRD 또는 GT 전용 가격/범위/사실관계는 일반화하지 않는다.
